DROP TABLE IF EXISTS review;
DROP TABLE IF EXISTS shipment;
DROP TABLE IF EXISTS productinventory;
DROP TABLE IF EXISTS warehouse;
DROP TABLE IF EXISTS orderproduct;
DROP TABLE IF EXISTS incart;
DROP TABLE IF EXISTS product;
DROP TABLE IF EXISTS category;
DROP TABLE IF EXISTS ordersummary;
DROP TABLE IF EXISTS paymentmethod;
DROP TABLE IF EXISTS customer;


CREATE TABLE customer (
    customerId          INT IDENTITY,
    firstName           VARCHAR(40),
    lastName            VARCHAR(40),
    email               VARCHAR(50),
    phonenum            VARCHAR(20),
    address             VARCHAR(50),
    city                VARCHAR(40),
    state               VARCHAR(20),
    postalCode          VARCHAR(20),
    country             VARCHAR(40),
    userid              VARCHAR(20),
    password            VARCHAR(30),
    PRIMARY KEY (customerId)
);

CREATE TABLE paymentmethod (
    paymentMethodId     INT IDENTITY,
    paymentType         VARCHAR(20),
    paymentNumber       VARCHAR(30),
    paymentExpiryDate   DATE,
    customerId          INT,
    PRIMARY KEY (paymentMethodId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE 
);

CREATE TABLE ordersummary (
    orderId             INT IDENTITY,
    orderDate           DATETIME,
    totalAmount         DECIMAL(10,2),
    shiptoAddress       VARCHAR(50),
    shiptoCity          VARCHAR(40),
    shiptoState         VARCHAR(20),
    shiptoPostalCode    VARCHAR(20),
    shiptoCountry       VARCHAR(40),
    customerId          INT,
    PRIMARY KEY (orderId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE 
);

CREATE TABLE category (
    categoryId          INT IDENTITY,
    categoryName        VARCHAR(50),    
    PRIMARY KEY (categoryId)
);

CREATE TABLE product (
    productId           INT IDENTITY,
    productName         VARCHAR(40),
    productPrice        DECIMAL(10,2),
    productImageURL     VARCHAR(100),
    productImage        VARBINARY(MAX),
    productDesc         VARCHAR(1000),
    categoryId          INT,
    PRIMARY KEY (productId),
    FOREIGN KEY (categoryId) REFERENCES category(categoryId)
);

CREATE TABLE orderproduct (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE incart (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE warehouse (
    warehouseId         INT IDENTITY,
    warehouseName       VARCHAR(30),    
    PRIMARY KEY (warehouseId)
);

CREATE TABLE shipment (
    shipmentId          INT IDENTITY,
    shipmentDate        DATETIME,   
    shipmentDesc        VARCHAR(100),   
    warehouseId         INT, 
    PRIMARY KEY (shipmentId),
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE productinventory ( 
    productId           INT,
    warehouseId         INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (productId, warehouseId),   
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE review (
    reviewId            INT IDENTITY,
    reviewRating        INT,
    reviewDate          DATETIME,   
    customerId          INT,
    productId           INT,
    reviewComment       VARCHAR(1000),          
    PRIMARY KEY (reviewId),
    FOREIGN KEY (customerId) REFERENCES customer(customerId)
        ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE CASCADE
);

INSERT INTO category(categoryName) VALUES ('Tutorials');
INSERT INTO category(categoryName) VALUES ('Laptop and Computer Accessories');
INSERT INTO category(categoryName) VALUES ('Headphones');
INSERT INTO category(categoryName) VALUES ('Smart Tech Accessories');
INSERT INTO category(categoryName) VALUES ('General Accessories');
INSERT INTO category(categoryName) VALUES ('Cell Phone Accessories');
INSERT INTO category(categoryName) VALUES ('Gaming Accessories');
INSERT INTO category(categoryName) VALUES ('TV Accessories');

INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Java Tutorial', 1, 'Learn Java with this great tutorial!',8.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Python Tutorial',1,'Python has never been this easy!',19.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Laptop Screen Protector',2,'Protect your laptops screen! (Laptop not included)',10.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Laptop Screen Cleaner',2,'Make sure your laptops screen is clean!',22.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Laptop Camera Cover',2,'High end camera cover',21.35);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Laptop Bag',2,'Stylish laptop bag',25.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Smart Watch Band',4,'High quality band',30.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Laptop Stand',2,'Create a comfortable work set up',40.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Keyboard Protector',5,'Protect your keyboard (Laptop Not Included)',97.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Cell Phone Charger',6,'Keep your phone full of charge',31.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Ear Bud Case',3,'Never lose your earphones again!',21.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Headphone Stand',3,'Something to rest your headphones on',38.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Smart Watch Charger',4,'Keep your watch full of charge!',23.25);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Laptop Sleeve',2,'Protect your laptop!',15.50);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Gaming Headphones',7,'Immerse yourself in the game!',17.45);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('USB Stick',5,'Over 8 GBs',39.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Cell Phone Case',6,'Protect your Phone!',62.50);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Gaming Controller Charger',7,'Lets you keep playing for hours',9.20);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Gaming Chair',7,'Play in a relaxing way!',81.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Gaming Mouse',7,'The best gaming mouse on the market',10.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Universal TV remote',8,'Make any TV yours!',21.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('R Tutorial',1,'This tutorial will make you love R!',14.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('HTML Tutorial',1,'This tutorial will help you master HTML!',18.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Cell Phone Screen Protector',6,'Keep your phone screen clean with this product',19.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Selfie Stick',6,'Take selfies from a little further away',18.40);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Charging Dock',6,'Charges multiple devices at the same time',9.65);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('TV Stand',8,'To improve your TV watching experience',14.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Laptop Cooling Pad',2,'To make sure your laptop does not overheat',21.05);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('JSP Tutorial',1,'Helps you get a good LAB 10 mark',14.00);

INSERT INTO warehouse(warehouseName) VALUES ('Main warehouse');
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (1, 1, 5, 18);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (2, 1, 10, 19);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (3, 1, 3, 10);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (4, 1, 2, 22);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (5, 1, 6, 21.35);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (6, 1, 3, 25);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (7, 1, 1, 30);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (8, 1, 0, 40);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (9, 1, 2, 97);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (10, 1, 3, 31);

INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Arnold', 'Anderson', 'a.anderson@gmail.com', '204-111-2222', '103 AnyWhere Street', 'Winnipeg', 'MB', 'R3X 45T', 'Canada', 'arnold' , 'test');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Bobby', 'Brown', 'bobby.brown@hotmail.ca', '572-342-8911', '222 Bush Avenue', 'Boston', 'MA', '22222', 'United States', 'bobby' , 'bobby');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Candace', 'Cole', 'cole@charity.org', '333-444-5555', '333 Central Crescent', 'Chicago', 'IL', '33333', 'United States', 'candace' , 'password');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Darren', 'Doe', 'oe@doe.com', '250-807-2222', '444 Dover Lane', 'Kelowna', 'BC', 'V1V 2X9', 'Canada', 'darren' , 'pw');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Elizabeth', 'Elliott', 'engel@uiowa.edu', '555-666-7777', '555 Everwood Street', 'Iowa City', 'IA', '52241', 'United States', 'beth' , 'test');
INSERT INTO customer (userid, password) VALUES ('admin1ad', '1adadmin');

-- Order 1 can be shipped as have enough inventory
DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (1, '2019-10-15 10:25:55', 91.70)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 1, 1, 18)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 2, 21.35)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 10, 1, 31);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (2, '2019-10-16 18:00:00', 106.75)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 5, 21.35);

-- Order 3 cannot be shipped as do not have enough inventory for item 7
DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (3, '2019-10-15 3:30:22', 140)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 6, 2, 25)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 7, 3, 30);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (2, '2019-10-17 05:45:11', 327.85)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 3, 4, 10)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 8, 3, 40)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 13, 3, 23.25)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 28, 2, 21.05)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 29, 4, 14);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (5, '2019-10-15 10:25:55', 277.40)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 4, 21.35)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 19, 2, 81)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 20, 3, 10);

-- New SQL DDL for lab 8
UPDATE Product SET productImageURL = 'img/java.jpg' WHERE ProductId = 1;
UPDATE Product SET productImageURL = 'img/python.jpg' WHERE ProductId = 2;
UPDATE Product SET productImageURL = 'img/laptop_screen_protector.jpg' WHERE ProductId = 3;
UPDATE Product SET productImageURL = 'img/laptop_screen_cleaner.jpg' WHERE ProductId = 4;
UPDATE Product SET productImageURL = 'img/laptop_camera_cover.jpg' WHERE ProductId = 5;
UPDATE Product SET productImageURL = 'img/laptop_bag.jpg' WHERE ProductId = 6;
UPDATE Product SET productImageURL = 'img/smart_watch_band.jpg' WHERE ProductId = 7;
UPDATE Product SET productImageURL = 'img/Laptop_Stand.jpg' WHERE ProductId = 8;
UPDATE Product SET productImageURL = 'img/Keyboard_Protector.jpg' WHERE ProductId = 9;
UPDATE Product SET productImageURL = 'img/phone_charger.jpg' WHERE ProductId = 10;
UPDATE Product SET productImageURL = 'img/earbud_case.jpg' WHERE ProductId = 11;
UPDATE Product SET productImageURL = 'img/headphone_stand.jpg' WHERE ProductId = 12;
UPDATE Product SET productImageURL = 'img/smartwatch_charger.jpg' WHERE ProductId = 13;
UPDATE Product SET productImageURL = 'img/laptop_sleeve.jpg' WHERE ProductId = 14;
UPDATE Product SET productImageURL = 'img/gaming_headphones.jpg' WHERE ProductId = 15;
UPDATE Product SET productImageURL = 'img/usb_stick.jpg' WHERE ProductId = 16;
UPDATE Product SET productImageURL = 'img/cell_phone_case.jpg' WHERE ProductId = 17;
UPDATE Product SET productImageURL = 'img/gaming_controller_charger.jpg' WHERE ProductId = 18;
UPDATE Product SET productImageURL = 'img/gaming_chair.jpg' WHERE ProductId = 19;
UPDATE Product SET productImageURL = 'img/gaming_mouse.jpg' WHERE ProductId = 20;
UPDATE Product SET productImageURL = 'img/universal_tv_remote.jpg' WHERE ProductId = 21;
UPDATE Product SET productImageURL = 'img/R.jpg' WHERE ProductId = 22;
UPDATE Product SET productImageURL = 'img/html.jpg' WHERE ProductId = 23;
UPDATE Product SET productImageURL = 'img/cellphone_screen_protector.jpg' WHERE ProductId = 24;
UPDATE Product SET productImageURL = 'img/selfie_stick.jpg' WHERE ProductId = 25;
UPDATE Product SET productImageURL = 'img/charging_dock.jpg' WHERE ProductId = 26;
UPDATE Product SET productImageURL = 'img/TV_stand.jpg' WHERE ProductId = 27;
UPDATE Product SET productImageURL = 'img/laptop_cooling_pad.jpg' WHERE ProductId = 28;
UPDATE Product SET productImageURL = 'img/jsp.jpg' WHERE ProductId = 29;

























-- Loads image data for product 1
UPDATE Product SET productImage =  WHERE ProductId = 1;